...
function getAge(age) {
  try {
    ...
    return { error: err.message, name: err.cause };
  }
...
function getBookById(books, id) {
  try {
    let book = books.find((value) => value["id"] === id);
    if (!book) {
...
function decodeURIComponentWrapper(encodedString) {
  try {
    let decodedStr = decodeURI(encodedString);
    return decodedStr;
  } catch (err) {
    if (err.name === "URIError") {
      return new Error("Помилка декодування URI").message;
    } else {
      return err.message;
    }
  }
